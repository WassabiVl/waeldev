  ...
  <q-card class="table-bg">
    <q-card-section>
      <div class="text-h6 text-white">
        Dark Mode
        <q-btn class="float-right text-capitalize  shadow-3" color="blue" icon="person" label="Export"/>
      </div>
    </q-card-section>
    <q-separator color="white"/>
    <q-card-section class="q-pa-none">
      <q-table :columns="columns" :rows="data" class="table-bg" dark hide-bottom>
        ...
        <template v-slot:body-cell-Action="props">
          <q-td :props="props">
            <q-btn dense flat icon="edit" size="sm"/>
            <q-btn class="q-ml-sm" dense flat icon="delete" size="sm"/>
          </q-td>
        </template>
        <template v-slot:body-cell-Progress="props">
          <q-td :props="props">
            <q-linear-progress :color="getColor(props.row.progress)" :value="props.row.progress/100" class="q-mt-md"
                               dark/>
          </q-td>
        </template>
      </q-table>
