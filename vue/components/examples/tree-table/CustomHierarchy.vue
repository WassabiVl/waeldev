    ...
    <span>
      <q-hierarchy id="test" :columns="columns" :data="data" classes="bg-indigo-8 text-white">
          <template v-slot:body="props">
            <td data-th="Name">
              <div :class="props.iconName(props.item)!='done'?'q-pl-lg':''"
                   v-bind:style="props.setPadding(props.item)">
                <q-btn v-if="props.iconName(props.item)!='done'" :icon="props.iconName(props.item)"
                       dense flat
            ...
            <td class="text-left">
              <q-chip v-if="props.item.note" class="text-white" color="lime-9" size="sm" square>
                {{ props.item.note }}
              </q-chip>
            </td>
          </template>
